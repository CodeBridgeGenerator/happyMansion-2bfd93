...
const [packingIds, setpackingIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("category")
                        ...
                        .then((data) => {
                        setcategoryIds(data.data);
                        });
client.service("brand")
                        .find({ query: { $select: ["_id","name","company"] } })
                        .then((data) => {
                        setbrandIds(data.data);
                        });
client.service("packing")
                        .find({ query: { $select: ["_id","type"] } })
                        .then((data) => {
                        setpackingIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = productsFakerFactory(props.user,fakerCount,categoryIds,brandIds,packingIds);
