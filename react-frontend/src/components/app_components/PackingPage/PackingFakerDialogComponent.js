...
import packingFakerFactory from "./packingFakerFactory";
...
const PackingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = packingFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("packing").create(elem));
...
export default connect(mapState, mapDispatch)(PackingFakerDialogComponent);
