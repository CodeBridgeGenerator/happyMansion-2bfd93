...
import brandFakerFactory from "./brandFakerFactory";
...
const BrandFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = brandFakerFactory(props.user,fakerCount,companyIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("brand").create(elem));
...
export default connect(mapState, mapDispatch)(BrandFakerDialogComponent);
