...
const BrandCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,company: _entity?.company?._id,
        ...
        const result = await client.service("brand").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Brand updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Brand" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Brand" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="brand-create-dialog-component">
...
export default connect(mapState, mapDispatch)(BrandCreateDialogComponent);
