...
import weightFakerFactory from "./weightFakerFactory";
...
const WeightFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = weightFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("weight").create(elem));
...
export default connect(mapState, mapDispatch)(WeightFakerDialogComponent);
